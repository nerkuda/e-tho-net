...
> в [`docs/`](docs/) — **источник истины** (авторитетнее кода). MVP собран; проект
> в режиме **тестовой эксплуатации и баг-фиксов**: пользователь сообщает об
> ошибках/замечаниях, агент воспроизводит, диагностирует, чинит и проверяет.
> Статусы и карта фаз — в [`docs/workplan.md`](docs/workplan.md) (индекс),
> описания задач — в `docs/workplan/`. Где что искать — см. §2 и §10.

## 0. Текущий режим работы

- **MVP сдан.** Фазы A–K и N — `done`; L/M — `in_progress` (остались ручные
  проверки L15/L16 и отложенный M8). Workplan — историческая справка и индекс,
  а не очередь работ.
...
2. **Не начинай новую функциональность «мимо плана».** Текущая работа — баг-фиксы
   из отчётов пользователя. Всё крупное получает новый ID (в файл фазы и индекс).
...
5. **Читай документацию «лениво».** `docs/workplan.md` — только индекс: общие
   принципы и статусы фаз (таблица в §3). Описания задач, DoD и решений по фазам —
   в `docs/workplan/phase-*.md` (по одному файлу на фазу). Перед работой открывай
   только файлы нужных фаз; весь workplan целиком не читай.

## 2. Структура репозитория и карта для отладки

```
etn/
├── docs/        # спецификации (01..11) + README + workplan (индекс) + workplan/ (фазы) + руководства
...
- **Фикс оказался больше** → заведи ID: добавь задачу в `docs/workplan/phase-*.md`
  (файл своей фазы) и строку в таблицу `docs/workplan.md` §3.
