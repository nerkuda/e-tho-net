...
- **Слои связей на холсте** (`links.ts`): визуальные линии — **под** облачками,
  интерактивность (hover/click) — через прозрачные wide-линии в `.links-overlay-hit`
  **над** облачками, выделенная связь — в `.links-overlay-top`. **На hover
  перерисовывай только top-слой (`drawActive`)** — полный `clearSvg` между
  `mousedown` и `mouseup` убивает `click` (уже наступавший баг).
