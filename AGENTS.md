...
- **Каталог задач и ошибок — мыслесеть «Тест 1».** Пользователь ведёт каталог
  задач и ошибок проекта в мыслесети **«Тест 1»**: мысль **`ETN`** (тип «софт») —
  корень, её дети «Ошибки», «План разработки» и «Выполненные задачи разработки»,
  конкретные пункты — их дочерние мысли. Когда пользователь ссылается на
  «задачу/ошибку в мыслесети» — речь об этом поддереве; посмотреть его можно
  через MCP-инструменты `etn.*` (сеть «Тест 1», поддерево мысли `ETN`). Это
  **не единственный** источник отчётов: баги и пожелания приходят и прямо
  в диалоге.
...
### 3.3. Фиксация результата задач и ошибок из мыслесети (обязательно)

При выполнении задачи или исправлении ошибки, ведущихся в мыслесети
(«Тест 1»: поддерево `ETN` — «Ошибки», «План разработки»), после завершения
работы **в обязательном порядке**:

1. **Сделай коммит(ы)** изменений (§3.1/§3.2; правки спецификаций — отдельным
   `docs(...)`-коммитом).
2. **Создай хронологический комментарий** (`kind: "chronological"`) к мысли
   задачи/ошибки: краткое описание сделанного и ID коммита (или коммитов).
   Обязательные поля: `title` — короткий заголовок записи; `valid_from` и
   `valid_to` — границы периода действия записи, `valid_to` всегда равен
   `valid_from` (дата записи, ISO-8601); `body_md` — описание.
3. **Помети мысль неактуальной** (`etn.thoughts.set_active: false`) — как
   требуют конвенции «План разработки» («выполненные задачи помечаются
   неактуальными») и «Ошибки» («исправленные ошибки следует отмечать
   неактуальными»).
