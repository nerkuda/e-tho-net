 ...
 * `VALIDATION_ERROR` with `details.candidates` when the name matches more
 * than one type — genuinely possible here, since `idx_link_types_name_keys`
 * only enforces uniqueness of the `(name_forward, name_reverse)` **pair**, so
 * one type's forward label can collide with another type's reverse label.
 */
export function resolveLinkTypeIdByName(ndb: NetworkDb, name: string): string {
  ...
  if (rows.length === 0) {
    throw new EtnError('NOT_FOUND', `link type "${name}" not found`, { field: 'type', name });
  }
  if (rows.length > 1) {
    throw new EtnError('VALIDATION_ERROR', `link type name "${name}" is ambiguous`, {
      field: 'type',
      name,
      candidates: rows.map((r) => ({
        id: r.id,
        name_forward: r.name_forward,
