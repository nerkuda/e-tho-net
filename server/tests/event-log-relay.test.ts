...
/**
 * Wait until `predicate()` holds or `timeoutMs` elapses. The relay polls on a
 * short interval, but under a fully parallel test run a fixed sleep is not
 * enough — the timer may be delayed far beyond the poll interval.
 */
...
describe('EventLogRelay (unit, fake log)', () => {
  it('never broadcasts pre-start history', async () => {
    ...
    try {
      relay.start();
      await waitUntil(() => relay.pollCount() >= 2);
      relay.stop();
  ...
  it('never re-broadcasts in-process writes', async () => {
    ...
    try {
      ...
      await waitUntil(() => relay.pollCount() >= 1);
      relay.stop();
