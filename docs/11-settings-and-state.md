...
| **L4+tab. Клиент × пользователь × сеть × таб** | локальный SQLite, `tabs` + `ui_state:<key>:<tab_id>` + `focus_history.tab_id` | нет (только на этом клиенте) | per-tab focus, view, filter_state (structures/chronicle), focus_history (08-ui-spec.md §1, 07-client-electron.md §3.5/§3.6) |
...
- Смена сети — у каждой сети своя история (PK `focus_history` включает
  `network_id`).
- **С вводом табов (фаза Q)** у каждого таба **своя** история фокуса:
...
- лимит 50, как у истории фокуса; хранится в локальной таблице
  `structures_history` (PK `(profile_id, network_id, tab_id, thought_id)` —
  с фазой Q `tab_id` обязателен, до неё допускается `NULL`); текущая
  открытая мысль из истории исключается при отображении;
- удаление мысли на сервере (`thought.deleted`) вычищает запись так же, как
  из `focus_history`;
- панель истории в статус-баре показывает историю активного вида (т.е.
  историю активного таба на активном виде).
...
- локальная таблица `chronicle_history` (PK
  `(profile_id, network_id, tab_id, entry_kind, entry_id)` — с фазой Q
  `tab_id` обязателен, до неё допускается `NULL`); лимит 50, свежие в
  начале, повторное открытие поднимает запись (без дублей);
