...
его сетям. Текущий слой изменений (S10, §5.2c) — тоже пер-ключевое состояние:
ключуется парой `(network_id, api_keys.id)`, поэтому два ключа одного
пользователя держат независимые слой-сессии, и переключение слоя одним ключом
не задевает другой.
...
(компактность); человек и REST-клиент запрашивают `GET /networks/{id}` или
ресурс `etn://networks/{network_id}` когда сеть уже выбрана.

**MCP-агент читает `conventions` иначе.** MCP-протокол не подаёт агенту ни
отдельного инструмента `etn.networks.get`, ни ресурсы (`etn://…`) — в наборе
инструментов их нет вообще (ограничение клиента). Поэтому `conventions`
(единственное из двух «объёмных» полей, нужное практически всегда — правила
записи перед `create`/`update`) отдаётся прямо в ответе `etn.networks.structure`
(§5.1) — инструмент, который агент и так вызывает первым при ориентировке в
сети, лишнего вызова не появляется. `examples` в этот ответ по умолчанию не
попадает (образцы объёмны и нужны реже) — агент запрашивает их явно
параметром `include_examples: true` того же инструмента. REST-эндпоинт `GET
/networks/{id}` и ресурс `etn://networks/{network_id}` остаются как были —
это путь для человека/будущих MCP-клиентов, которые всё же подают ресурсы.
...
| `etn.networks.structure` | `read-only` | Структура сети: активные мысли узлового типа (превью постоянного комментария, свойства, счётчики, каталог типов) + `conventions` сети (§3). Когда `node_section_type_id` не задан — пустой `sections`, но `conventions` всё равно возвращается | `network_id`, `include_examples?` (по умолчанию `false` — добавляет `examples` в ответ) |
...
| `etn.thoughts.get` | `read-only` | Полная мысль (+ блок `meta`: счётчики связей/вложений/хроники, превью постоянного комментария; `thought_ref`-значения свойств резолвнуты в `{id, title}`; с 0.7.3 — `meta.views` эффективный набор отборов для мысли по цепочке типов) | `network_id`, `thought_id`, `view?` (`compact` — дефолт для MCP, `full` — task O12, см. §5.1e) |
...
| `etn.types.list` | `read-only` | Оба каталога типов целиком (не только использованные в другом ответе), с иерархией и эффективными свойствами — см. §5.1b; `scope` позволяет запросить один каталог отдельно. С 0.7.3 (задача c1fa71d4) каждый тип мысли несёт собственные `views[]` (без наследования от предков — эффективный набор для конкретной мысли идёт через `meta.views` в `etn.thoughts.get`) | `network_id`, `scope?` (`thoughts`/`links`/`all`), `in_subtree_of?`, `max_depth?` |
| `etn.views.run` | `read-only` | Исполнить именованный отбор типа относительно конкретной мысли (задача c1fa71d4, 0.7.3, операция cb8d8e43). `view_name` — нормализованное имя из `meta.views` карточки мысли или `id` отбора. Возвращает страницу мыслей + `meta.view` (отбор) + `meta.unresolved` (если токен в `definition` не разрешим на момент исполнения — тогда `data` пустое). Read-only: бюджет записи не тратит, `audit_log` не пишет | `network_id`, `thought_id`, `view_name`, `limit?`, `offset?`, `order?` |
| `etn.changes.list` | `read-only` | Дельта событий из `event_log` для долгоживущего агента с кэшем (`seq > since_seq`, с признаком усечения буфера, фильтром `audience` и фильтром видимости по слою сессии) — см. §5.1c | `network_id`, `since_seq`, `limit?` |
...
**`conventions`/`examples` (bug fix).** Ответ верхнего уровня несёт также
`conventions` сети (§3) — правила записи, которые агент должен прочитать
перед `create`/`update`/`upsert_bundle`; поле возвращается всегда (`null`,
если не заполнено), включая случай `has_structure: false` — оно не связано
с узловым типом. `examples` (образцы «хорошо/плохо») в ответ по умолчанию
не входит — это объёмное поле, нужное реже; агент запрашивает его явно
параметром `include_examples: true`. Так закрывается разрыв: до этого
`conventions`/`examples` были доступны только через `GET /networks/{id}`
или ресурс `etn://networks/{id}`, а MCP-агент не имеет доступа ни к
`etn.networks.get` (такого инструмента нет), ни к MCP-ресурсам
(ZCode-клиент их агенту не подаёт).
...
- **Фильтр `audience`.** События с `audience: "user"` (например,
  ...
  §5).
- **Фильтр слоя (S9).** События с `audience: "network"` дополнительно
  фильтруются по слою сессии вызывающего пользователя — тому же правилу
  видимости, что у WebSocket-шлюза ([04-realtime.md](04-realtime.md) §11.2):
  агент видит только события своей цепочки слоёв и не получает события по
  перекрытым им строкам. Координата сессии — `(user_id, '')` (MCP не передаёт
  `Client-Id`; пока инструмент выбора слоя не реализован — S10 — сессия
  агента сидит в основе, и фильтр пропускает ровно события основы плюс
  события слоёв, которые агент перекрыл бы, — то есть пока это поведение
  тождественно старому). `layer_id` каждого элемента — поле конверта события,
  для агента информативно: из какого слоя пришло изменение.
...
- **Усечение при смене слоя (S9).** `truncated: true` также выдаётся, когда
  `since_seq` предшествует последнему переключению слоя сессии агента
  (`session_layers.switched_at_seq`, миграция 028;
  [04-realtime.md](04-realtime.md) §11.3): дельта через границу переключения
  смешала бы два разных фильтра видимости. После полной ресинхронизации агент
  продолжает опрос с `cursor.max_seq` последнего ответа — позиции
  `>= switched_at_seq` уже не усекаются по этой причине. `since_seq = 0`
  («от начала буфера») по этой причине тоже не усечён.
- **`data.db` открывается.** Сами события живут в `_system.db`, но слой сессии
  и проверка видимости требуют сети `data.db` — инструмент открывает
  соединение слоя сессии (`openNetworkDb`), членство по-прежнему проверяется
  через `systemDb.getMemberRole`.
...
- `network_id` — обязательный. Доступ — любой член сети (`member` или
  `owner`), либо глобальный админ (`is_admin`) без явного членства
  (06-auth.md §4.1).
...
**Семантика `direction` у inline-связей.** Поле `direction` инструментов
`etn.thoughts.create` (`link`) и `etn.thoughts.upsert_bundle` (`links[]`)
называет роль `target_thought_id` для НОВОЙ мысли:
...
Семантика единая для REST и MCP: REST-поле `create_link.direction`
(03-server-api.md §6.3) означает то же самое, перевода значения на границе
MCP-слоя нет. `etn.links.create` семантики `direction` не имеет вовсе — там
связь задаётся явно `source_id` (родитель) → `target_id` (ребёнок).
...
ошибка валидации любого ключа откатывает весь набор. Для свойств с
`config.multiple` значение может быть массивом: `thought_ref` — массив id
(пустой массив очищает, одиночный id нормализуется в массив из одного);
`url` — массив URL/путей (пустой массив очищает, одиночная строка
нормализуется в массив из одного, дубликаты схлопываются; задача 0.6.2).
Без флага `multiple` массив отвергается (422). Ответ одиночной формы —
...
> **Коэрс строковых скаляров в одиночной форме** (баг-фикс 0.5.3). Некоторые
> MCP-хосты (например, ZCode) приводят скалярный union-параметр `value` к
> строке — `value: true` доходит до сервера как `"true"`, и доменная валидация
> честно отвергала его (422 `expects a boolean`). Вложенные объекты
> (`values`, `properties` у `upsert_bundle`) такие хосты не искажают, поэтому
> касается это только одиночной формы `key` + `value`. На границе MCP (до
> доменной валидации) выполняется коэрс по `value_type` найденного определения:
> - `bool` — строки ровно `"true"`/`"false"` (без учёта регистра) → `true`/`false`;
> - `number` — строки, которые `Number()` разбирает в конечное число
>   (`"42"`, `"3.5"`, `"1e3"`) → число; пустая строка и не-числа (`"abc"`,
>   `"Infinity"`) коэрсом не проходят и отвергаются валидацией как раньше.
> Текстовые типы (`text`, `url`, `date`) не коэрсятся никогда: `"true"` в
> текстовом свойстве остаётся строкой. Доменная валидация
> (`property-service.ts`) осталась строгой — коэрс живёт только в MCP-фасаде.
...
#### 5.2c. Слои изменений (задача S10)

Слои — copy-on-write «плёнка» поверх живой основы (13-layers.md). Агент
получает **полный паритет** с пользователем в клиенте и с REST (§10.2):
шесть управляющих инструментов плюс сквозная слоистость всех остальных.

| Tool | Аннотации | Описание | Параметры |
|------|-----------|----------|-----------|
| `etn.layers.list` | `read-only` | Все слои сети с иерархией: `parent_id`, `depth`, `comment`, `git_branch`, `children_count` (число для подтверждения `cascade` при удалении) и `current: true` у слоя вызывающего ключа | `network_id`, `include_service?` |
| `etn.layers.diff` | `read-only` | Структурное отличие слоя от цели (S11, §10.3): `links` — added/removed/type_changed/reparented/reorder_collapsed; `overridden` — id, физически присутствующие в слое (тени, вставки, надгробия) | `network_id`, `layer_id` |
| `etn.layers.diff_doc` | `read-only` | Содержательное отличие: два детерминированных markdown-документа (`layer_doc`/`target_doc`) для построчного сравнения (плоская сборка до T2) | `network_id`, `layer_id` |
| `etn.layers.create` | — | Создать слой; `parent_id` по умолчанию — текущий слой вызывающего ключа. `comment` необязателен, но настоятельно рекомендуется — по нему следующий агент поймёт назначение слоя, не расспрашивая автора (§2.2). Сессию на новый слой НЕ переключает — для этого `etn.layers.select` | `network_id`, `title`, `parent_id?`, `comment?`, `git_branch?` |
| `etn.layers.update` | `idempotent` | Переименовать слой и/или править комментарий; название основы фиксировано (422 при попытке переименовать) | `network_id`, `layer_id`, `title?`, `comment?`, `expected_version?` |
| `etn.layers.delete` | `destructive` | Удалить слой вместе со всем поддеревом потомков (§2.4). При `children_count > 0` обязан `cascade` с этим числом (из `etn.layers.list`); несовпадение — 409, отсутствие — 422 с `{children_count}`. Удаляет теневые строки и надгробия физически (это не слияние — предку ничего не переносится) и сразу автоочищает корзину | `network_id`, `layer_id`, `cascade?` |
| `etn.layers.select` | `idempotent` | Переключить текущий слой вызывающего ключа — все последующие вызовы этого ключа (чтения и записи) идут в новом слое. Служебный (резервный) слой выбрать нельзя. Повторный выбор того же слоя — no-op | `network_id`, `layer_id` |
| `etn.layers.merge` | `destructive` | Слить слой в родителя: полностью (без `tables`) или частично замкнутым подмножеством. Любой конфликт `base_version` или незамкнутость набора отклоняет ВСЮ операцию (422 с `conflicts`/`missing_closure`) — частичного применения не бывает. Успех: `{applied, skipped, reorder_collapsed, reserve_layer_id, purged}` (§8.3); эмитится ровно одно событие `layer.merged` с атрибуцией на цель слияния | `network_id`, `layer_id`, `tables?` (`{таблица: [id…]}`) |

**Сквозное свойство, а не отдельные инструменты (§10.2).** Все существующие
инструменты чтения и записи (`etn.thoughts.*`, `etn.links.*`, поиск, подграф,
`etn.comments.*`, свойства, bundle…) работают **в контексте текущего слоя
вызывающего ключа** — отдельных «послойных» вариантов инструментов нет.
Слой сессии хранится в той же таблице `session_layers`, что и у REST (§7.1),
но ключуется иначе: у MCP нет понятия `Client-Id` (§2.2 — агент лишь передаёт
`network_id`), поэтому идентичность сессии — `mcp:<api_keys.id>`. Следствия:

- у каждого API-ключа своя независимая текущая слой-сессия (два ключа одного
  пользователя не мешают друг другу — как два REST-клиента с разными
  `Client-Id`); это же фундамент будущего «ключ, прибитый к слою-песочнице»
  (§10.2, ценность отмечена в S10);
- `etn.layers.list` помечает `current` по ключу;
- `etn.changes.list` фильтрует события видимостью в слое ключа (§12) и
  отдаёт `truncated: true`, когда `since_seq` раньше последнего переключения
  слоя.

**Эхо слоя (§7.1).** Ответ каждого пишущего инструмента (все 17
существующих + `layers.*`) содержит поле `layer: { id, title }` — текущий
слой вызывающего ключа на момент ответа. Агент, который пишет не в тот слой,
видит это сразу в результате, а не по последствиям. Ответы на чтение эха не
несут — они и так неявно связаны со слоем сессии (исключение — сам список
слоёв с флагом `current`).

Полный цикл агента одной командой за другой, без REST руками: `etn.layers.list`
→ `etn.layers.create` → `etn.layers.select` → обычные записи/чтения →
`etn.layers.merge` → `etn.layers.delete` (после удаления слоя сессия ключа
автоматически переключается сервером на родителя удалённого — отдельный
`select` обратно не нужен).
