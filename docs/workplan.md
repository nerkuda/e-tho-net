...
- **DoD:**
  - [ ] Разделители создаются в DOM холста, стилизованы (hover/активный),
    поверх SVG-слоёв связей.
  - [ ] Драг не запускает полный re-render на каждое движение (только
    CSS-переменные; зоны/связи пересобираются через ResizeObserver).
  - [ ] Сохранение/восстановление `canvas_layout`, сброс двойным кликом.
  - [ ] Тесты `parseCanvasLayout`; `npm run typecheck` зелёный; ручная
    проверка драга с занятым холстом.

### L9. Холст: масштабирование Ctrl+± (облачка, фокус, надписи связей)
- **Статус:** `in_progress` · **Assignee:** zcode · **Зависимости:** L8
...
- **DoD:**
  - [ ] Клавиши работают на экране сети; зум пошаговый с клипом 0.5–2.0.
  - [ ] Все масштабируемые размеры (CSS-переменные, формулы pure.ts, SVG
    links.ts) согласованы: сетка не расходится с фактической высотой DOM.
  - [ ] Сохранение/восстановление `canvas_zoom`; индикатор в статус-баре.
  - [ ] Тесты `parseCanvasZoom`/`zoomStep`/геометрии с zoom; typecheck зелёный;
