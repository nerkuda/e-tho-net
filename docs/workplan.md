...
- **Статус:** `in_progress` · **Assignee:** zcode · **Зависимости:** L10, L11
- **Описание:** пять правок холста по замечаниям пользователя:
  ...
  2) компоновка: иконка — вертикальная полоса на всю высоту слева, крупнее;
     ...
     краю заголовка;
  3) разделители зон — 4 px, в покое прозрачные (тонкая линия-хинт 1 px),
     слоем **под** связями (hit/top оверлеи выше), подсветка при hover/drag;
  ...
  5) фон холста — градиентная полоса фокуса: горизонтальная полоса
     ...
     краям холста; центр полосы следует за фокус-рядом (переменные
     `--focus-band-*` на host, обновляются при рендере и ресайзе).
- **DoD:**
  - [ ] Эллипсы визуально на рамках; drag/подсветки/drop работают; линии
    крепятся к центрам эллипсов.
  - [ ] Формула `cloudHeight()` синхронна новой компоновке; сетка/виртуализация
    не расходятся с DOM; зум 50–200%.
  - [ ] Сплиттеры не перекрывают связи визуально; drag/double-click-reset
    работают.
  - [ ] Авто-контраст на тёмных и светлых пользовательских фонах (тест
    `contrastText`); заданный явно цвет текста всегда побеждает.
  - [ ] Полоса фокуса следует за фокус-рядом (рендер, сплиттеры, ресайз
    окна); в обеих темах.
