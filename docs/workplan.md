...
- **DoD:**
  - [ ] Пакет `markdown/` (`@etn/markdown`) в workspaces + скрипты сборки/тестов.
  - [ ] Юнит-тесты рендера: wiki-ссылки/алиасы, размеры картинок, подсветка,
    таблицы, XSS-пробы (`<script>`, `javascript:`), лимит длины.
  - [ ] Сервер использует `@etn/markdown`; старый `domain/markdown.ts` удалён.
  - [ ] Sweep перерендера `body_html` при старте, версия рендера в `settings`.
  - [ ] Typecheck + тесты зелёные.

### M2. Фундамент: CodeMirror 6 вместо textarea
- **Статус:** `todo` · **Assignee:** zcode · **Зависимости:** M1
...
- **DoD:**
  - [ ] CM6-редактор в `markdown-field.ts`; старый textarea-путь удалён.
  - [ ] Все потребители (постоянный комментарий, хроника, вложения)
    работают без изменений их кода.
  - [ ] Typecheck + клиентские тесты зелёные.

### M3. Wiki-ссылки `[[имя|алиас]]`: синтаксис, автокомплит, переход
- **Статус:** `todo` · **Assignee:** zcode · **Зависимости:** M1, M2
...
- **DoD:**
  - [ ] Парсинг и подсветка `[[…]]` в CM6.
  - [ ] Автокомплит по префиксу (1+ символ), вставка с закрывающим `]]`.
  - [ ] Клик по wiki-ссылке в просмотре — фокус мысли; «не найдено» — уведомление.
  - [ ] Рендерер: alias-вывод, экранирование, вложенность не ломает парсинг.
  - [ ] Typecheck + тесты зелёные.
...
- **DoD:**
  - [ ] Подсветка ` ```ts ` и др. языков в редакторе и просмотре.
  - [ ] Тёмная тема: классы подсветки читаемы.
  - [ ] Typecheck + тесты зелёные.
...
- **DoD:**
  - [ ] Рендерер: px/%, экранирование, невалидный размер — обычная картинка.
  - [ ] Просмотр: картинка масштабируется по указанному размеру.
  - [ ] Typecheck + тесты зелёные.

### M6. Live preview: скрытие синтаксиса и блочные виджеты
- **Статус:** `todo` · **Assignee:** zcode · **Зависимости:** M3–M5
...
- **DoD:**
  - [ ] Скрытие маркеров с появлением при активации строки/блока.
  - [ ] Виджеты таблиц/кода/картинок; клик переводит блок в редактирование.
  - [ ] Wiki-ссылки отображаются как ссылки (скобки скрыты).
  - [ ] Производительность: декорации пересобираются только при изменениях.
  - [ ] Typecheck + тесты зелёные.

### M7. Mermaid-диаграммы
- **Статус:** `todo` · **Assignee:** zcode · **Зависимости:** M6
- **Описание:** ` ```mermaid ` — виджет с рендером mermaid.js в редакторе и
  в HTML-просмотре (сервер и клиент используют единый конвейер).
- **DoD:**
  - [ ] Mermaid рендерится в просмотре и в редакторе (виджет).
  - [ ] Ошибки синтаксиса показываются, не роняя редактор.
  - [ ] Typecheck + тесты зелёные.
