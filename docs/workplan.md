...
- **DoD:**
  - [ ] Сервер: `GET /networks/{nid}/thoughts/{id}/usage` (обратный поиск
  ...
  - [ ] IPC `thoughts.usage`, `attachments.getContent/updateContent`.
  - [ ] Клиент: оболочка с вкладками (регистр контента по вкладкам, keep-alive
    в рамках рендера, sticky-таббар), `group.ts`: lazyCount/`etn:set-count`/
    compact-подгруппы; переиспользуемый сплиттер.
  - [ ] Вкладки: «Основное» (свойства без заголовков колонок ≤5 строк,
  ...
  - [ ] Спеки 08 §6.3–6.7, 03 §9.1/§11 обновлены; user-guide дополнен.
  - [ ] Тесты: `findThoughtUsage`, attachment content (GET/PUT, не-текстовый
    mime → 400); `npm run typecheck` + тесты зелёные; ручная проверка всех
    вкладок, сплиттеров, ленивости и realtime.
