...
- **DoD:**
  - [x] Правка постоянного комментария и заголовка мысли зеркалится в
    ...
    при открытии — восстановление черновика.
...
- **Описание:** тёмная тема (08-ui-spec.md §13) поверх существующих
  ...
  (11-settings §2.1, per-installation, оба значения light/dark), новый
  IPC-домен `meta.get`/`meta.set`; применяется на boot до показа экранов
  (включая фон окна Electron). Переключатель — чекбокс «Тёмная тема» в меню
  «Вид» (§8). Заодно: фон холста — точечная сетка (dot grid), плавные
  hover-переходы (120 мс, отключаются при `prefers-reduced-motion`).
- **DoD:**
  - [ ] Переключение живьём: холст, панели, редактор, диалоги, меню, поиск,
    бейджи, `<mark>` — без белых протечек в тёмной теме.
  - [ ] Тема сохраняется в `client_meta.theme` и восстанавливается на boot
    (фон окна соответствует теме); работает на всех экранах, не только сети.
  - [ ] `meta.get`/`meta.set` в IPC-контракте, preload и типах; typecheck
    зелёный; клиентские тесты зелёные.

### L11. Редизайн облачка: компактнее и «дороже»
- **Статус:** `todo` · **Assignee:** zcode · **Зависимости:** L10
- **Описание:** визуальное обновление облачка без изменения состава и
  ...
  как в §2.2): скругление 14 px (было 8), мягче рамка (`--border` вместо
  `--border-strong`), двухслойная тень, hover — подсветка рамки + тень
  (без transform, чтобы не конфликтовать с FLIP-анимацией), круглая иконка-
  медальон 1.9 em (вместо квадрата 2.2 em), заголовок line-height 1.35
  (было 1.5), строка индикаторов компактнее (0.8 em). Фокус-облачко —
  свечение акцентом (`--shadow-focus`). Формула `cloudHeight()` в pure.ts
  синхронно обновлена (TITLE_LINE_FACTOR 1.35, IND_LINE_FACTOR 1.1) — сетка
  виртуализации не расходится с фактической высотой DOM.
- **DoD:**
  - [ ] Виртуализация и линии связей сходятся с фактической геометрией
    (скролл, зум 50–200%, FLIP-переход) — расхождение ≤2 px.
  - [ ] Пользовательские цвета/шрифты, dim, выделение, drag эллипсов и
    drop-подсветки работают как раньше.
  - [ ] Тесты геометрии обновлены; typecheck зелёный; ручная проверка в обеих
    темах.
