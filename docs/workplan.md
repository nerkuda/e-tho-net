...
- **Описание:** хром интерфейса (тулбар, статус-бар, панель истории фокуса,
  кнопки-шестерёнки, каретки ▾, крестик закрытия диалога) переводится с
  ...
  трогаются. Индикатор соединения — CSS-точка (цвет по состоянию), не эмодзи.
- **DoD:**
  - [ ] Тулбар/статус-бар/история/⚙/каретки/× — SVG, наследуют цвет темы
    (`currentColor`), корректные размеры.
  - [ ] Индикатор соединения — CSS-точка с цветами по состоянию и tooltip.
  - [ ] Контентные эмодзи (иконки мыслей, 📝/📅/📎 в облачках и поповерах
    связей) не затронуты.
  - [ ] Typecheck + клиентские тесты зелёные.

### L14. Связи: кривые Безье с градиентом источник→цель
- **Статус:** `todo` · **Assignee:** zcode · **Зависимости:** L12
- **Описание:** прямые `<line>` в оверлее связей заменяются кубическими
  Безье `<path>`: контрольные точки уходят по нормалям точек крепления
  (низ источника → вниз, верх цели → вверх), изгиб ограничен. Штрих —
  линейный градиент от цвета источника к цвету цели (собственный/типовой
  фона мысли; при отсутствии — цвет типа связи; при совпадении — сплошной).
  Слои и правила §7 сохраняются: визуал под облачками, wide-path hit-линии
  над ними (та же геометрия), hover — только top-слой. Бейдж-счётчик и
  подписи — в точке t=0.5 кривой.
- **DoD:**
  - [ ] Геометрия `edgeGeometry()` чистая и покрыта тестом (изгиб в клипе,
    середина кривой на отрезке Безье).
  - [ ] Hit-слой повторяет кривую — hover/click/контекстное меню работают;
    полный redraw между mousedown/mouseup не происходит (drawActive only).
  - [ ] Градиенты создаются в `<defs>` базового svg на каждый draw,
    дедупликация по паре цветов; тёмная/светлая темы без протечек.
  - [ ] Дэши (solid/dashed/dotted) и зум (толщины ×zoom) работают на путях.
  - [ ] Typecheck + клиентские тесты зелёные.
