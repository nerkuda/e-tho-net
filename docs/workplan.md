...
- **DoD:**
  - [ ] Тулбар: сегмент из двух SVG-кнопок с тултипами; скрытие холста и
    строки поиска в виде «Структуры»; редактор/статус-бар остаются.
  - [ ] Сервер: `POST /thoughts/query`, `GET /thoughts/{id}/hierarchy`,
    CRUD `/saved-filters` (миграция 016, `UNIQUE(user_id, name)`), события
    `saved-filter.*` (`audience=user`).
  - [ ] Отбор применён по кнопке/Enter; пустой фильтр возвращает только HOME;
    ключевые слова — `*`-маска, `-`-исключение, AND; парсер в shared с
    клиентскими тестами.
  - [ ] Дерево: раскрытие вверх/вниз, дедуп в ветке, лимит 100 соседей на
    узел, коннекторы с подписями типов, hover/клик/контекст-меню связей.
  - [ ] Редактор открывает мысль по `editorTarget {kind:'thought', id}` без
    смены фокуса; полоса активной мысли; отдельная история вида.
  - [ ] Спеки 02/03/04/08/11 дополнены; typecheck + клиентские тесты зелёные;
    ручная проверка обоих видов.
