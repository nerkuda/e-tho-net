...
- имеет прежние естественные UNIQUE-ограничения **в пределах слоя и среди
  живых строк** — с `layer_id` в составе (например `links (source_id,
  target_id, type_id, layer_id)` — с миграции 029 это partial-индекс
  `WHERE deleted = 0`: надгробие связи тройку освобождает, иначе повторное
  создание той же тройки в слое упиралось бы в собственное надгробие,
  S14/13-layers.md §6; глобальная уникальность связей проверяется после
  разрешения слоёв). `thought_synonyms` и `comment_targets`, не имевшие
  собственного id, получили `id` (UUID, `DEFAULT gen_uuid()` — SQL-функция,
  регистрируемая сервером на соединении).

**Неветвимые** (без `layer_id`, общие для всех слоёв): `layers`,
`session_layers` (умолчание слоя клиентской сессии — S7, ниже §3.10.7),
...
**Проверка из рабочего слоя** (S11): теневая строка **самого текущего слоя**
удержанием не считается — это и есть проверяемая строка (иначе мысль,
созданная в слое, блокировала бы сама себя). Живая строка **основы**
удерживает: «удалить совсем» в слое — лишь надгробие
([13-layers.md](13-layers.md) §5.2), поэтому для строки, существующей в
основе, в слое разрешена только пометка на удаление; такое удержание
возвращается элементом `blocking.layers` с `id` основы (03-server-api.md
§6.5a).
...
| `config` | TEXT | JSON-конфигурация свойства. Ключи: `default_value` — значение по умолчанию (L6, не задаётся для `thought_ref`); для `value_type = 'text'` — `options` (предустановленный список значений — подсказка ввода, не ограничение) и `multiple` (разрешить несколько значений через запятую); для `value_type = 'thought_ref'` — `multiple` (разрешить несколько ссылок: значение — массив id, хранится JSON-массивом в `value_thought_ref`, §3.5) и `allowed_type_id` (легаси-форма: один тип) / `allowed_type_ids` (список типов: поиск при заполнении идёт только по ним; при записи каждый id проверяется по этому списку); для `value_type = 'url'` — `multiple` (разрешить несколько URL/путей: значение — массив строк, хранится JSON-массивом в `value_text`, §3.5; используется именно JSON-массив, а не comma-join как для `text`, потому что URL может содержать запятую). Смена `options` / `multiple` / `allowed_type_ids` никогда не обрабатывает уже сохранённые значения |
| `required` | INTEGER NOT NULL DEFAULT 0 | |
| `position` | INTEGER NOT NULL DEFAULT 0 | Порядок отображения |
| `description` | TEXT | Свободное описание свойства: что оно значит и в каком формате значение. Показывается подсказкой рядом со свойством в редакторе мысли (ⓘ + tooltip) и отдаётся AI-агентам в `etn.types.list`. Наследуется подчинёнными типами вместе со свойством; переопределение — `type_property_overrides.description` (§3.7.1) |
...
- Подчинённый тип **не может** изменить `value_type`/`config`/`required` и
  ...
  умолчанию** и **описание** для себя (`type_property_overrides`, §3.7.1;
  `PUT …/properties/{id}/default`, `PUT …/properties/{id}/description`;
  `null` в теле — сброс соответствующего переопределения). Эффективное
  описание в списке свойств — переопределение типа, иначе `description`
  определения; флаг `description_overridden` различает эти случаи.
...
| `value_text` | TEXT | Для `value_type = 'text'` — обычная строка; для `value_type = 'url'` — URL/путь (`http(s)://…`, `file://…`, локальный путь). Для `url` с `config.multiple` — JSON-массив строк (`["https://a","file://b"]`); одиночное значение хранится «сырой» строкой. При чтении массив всегда разбирается в `string[]` (форма данных важнее флага), одиночная строка у multiple-свойства читается как массив из одного элемента |
...
Множественные `url` (`config.multiple`, задача 0.6.2): запись принимает массив
URL/путей к файлам (пустой массив очищает значение, одиночная строка
нормализуется в массив из одного); дубликаты схлопываются; каждый элемент
должен быть строкой. Массив отвергается, если у определения нет `config.multiple`.
Хранится JSON-массивом в `value_text` (а не comma-join, как `text`): URL может
содержать запятую, и обратный разбор склеенной строки был бы неоднозначен.
Уже сохранённое одиночное значение при включении `multiple` нормализуется
в массив из одного (правило «stored shape wins» для одиночных записей).
...
| UNIQUE | `(source_id, target_id, type_id, layer_id)` **среди живых строк** | Запрет дублирования связей того же типа между той же парой — **в пределах слоя** (S2, 13-layers.md §3); с миграции 029 (S14) — partial-индекс `idx_links_triple_live WHERE deleted = 0`: надгробие связи тройку освобождает, иначе повторное создание той же тройки в слое после удаления упиралось бы в собственное надгробие (13-layers.md §6). Глобальная уникальность проверяется после разрешения слоёв. Нетипизированные (`type_id IS NULL`) дубли по-прежнему не ловятся UNIQUE (NULL-семантика SQLite) — их отвергает приложение |
...
`idx_links_triple_live` (частичный UNIQUE тройки, S14/миграция 029).
...
Переопределения унаследованных свойств: значение по умолчанию и описание. Тип
не может изменить тип значения или удалить родительское свойство — только
подставить свой `default_value` и/или `description` для себя (и, транзитивно,
для своих подчинённых, пока те не переопределят сами). Для собственных
свойств типа override не хранится — дефолт живёт в `config.default_value`,
описание — в `type_properties.description` определения.

Одна строка может переопределять оба аспекта сразу или только один. Сброс
одного аспекта не трогает второй; строка, не переопределяющая ничего,
удаляется.
...
| `layer_id` / `deleted` / `base_version` | | Слой-колонки (§3.0.1) |
| `owner_type` | TEXT NOT NULL | `'thought_type'` \| `'link_type'` |
| `type_id` | TEXT NOT NULL | Тип, переопределяющий аспект унаследованного свойства |
| `property_id` | TEXT NOT NULL | Унаследованное свойство; SQL-FK снят (S2, §3.0.1) — чистит приложение |
| `default_value` | TEXT NOT NULL | Значение по умолчанию, JSON-кодированное (string/number/boolean). Строка `'null'` (JSON null) означает «дефолт не переопределён» — так выглядит строка, переопределяющая только описание; `NULL` в SQL не используется |
| `description` | TEXT | Переопределение описания свойства (`type_properties.description`). `NULL` — описание не переопределено (наследуется от определения) |
...
#### 3.10.7. session_layers (S7)

Умолчание «текущий слой сессии» — серверное скрытое состояние клиента
(13-layers.md §7.1): сессия идентифицируется тройкой
`(user_id, client_id, network_id)` — заголовок `Client-Id` REST-запросов
([11-settings-and-state.md](11-settings-and-state.md) §1.2); `client_id = ''`
означает вызов без `Client-Id` (агент, скрипт). Все чтения и записи сессии
выполняются в контексте записанного слоя; эхо `meta.layer` в каждом пишущем
ответе (03-server-api.md §5a.2) делает скрытое состояние обнаружимым.

| Столбец | Тип | Описание |
|---------|-----|----------|
| `user_id` | TEXT NOT NULL | |
| `client_id` | TEXT NOT NULL DEFAULT '' | `Client-Id` установки клиента |
| `layer_id` | TEXT NOT NULL | Текущий слой; SQL-FK нет — удаление слоя переприцепляет записи приложения (13-layers.md §2.4) |
| `updated_at` | TEXT NOT NULL | |
| PRIMARY KEY | `(user_id, client_id)` | Одна запись на сессию |

Запись отсутствует или указывает на несуществующий слой → сессия работает в
основе (защита от гонок, деградация вместо ошибки каждого запроса).
