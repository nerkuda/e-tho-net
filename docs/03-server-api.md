...
Реализация: title и синонимы без `*` — подготовленный запрос по FTS
(перечисление токенов через OR). Синонимы с `*` — wildcard-паттерны: FTS-запросы
по литеральным префиксам дают кандидатов, точную семантику проверяет
регулярное выражение (02-data-model.md §3.2: `*` не пересекает границу слова,
