...
- **DoD:**
  - [ ] Сервер: таблица `user_pinned_thoughts` (миграция 018, FK ON DELETE
    ...
    серверные тесты.
  - [ ] Клиент: IPC-домен `pins`; панель в тулбаре (чипы со стилями мысли,
  ...
  - [ ] Строка поиска + шестерёнка — под верхней строкой на карте мыслей,
    скрыты в виде «Структуры»; `Ctrl+F` по-прежнему открывает поиск на карте.
  - [ ] Спеки 02/03/04/08/11 и user-guide дополнены; typecheck + тесты зелёные.
