...
## L19. Упростить добавление мыслей: клик по зоне, меню зоны, Ctrl+Shift+Enter
- **Статус:** `in_progress` · **Assignee:** zcode · **Зависимости:** —
- **Источник:** задача «Упростить добавление мыслей» (мыслесеть «Тест 1»,
  `ETN → План разработки`).
- **Описание:** три новых пути добавления мыслей на холсте: (1) двойной клик по
  пустому месту зоны «Верх-лево» открывает диалог добавления родительской
  мысли, зоны «Низ» — дочерней (владелец вызова — фокус; одиночный клик по
  пустому месту побочных эффектов не имеет — по замечанию пользователя после
  ручной проверки); в «Верх-право» добавления нет — у фокуса может быть
  несколько родителей; (2) в контекстном меню зон «Верх-лево»/«Низ» появился
  пункт «Добавить мысль» (в меню «Верх-право» пункта нет, сортировка — как
  раньше); (3) `Ctrl+Shift+Enter` в диалоге добавления вставляет всё и сразу
  переводит в фокус первую из добавляемых мыслей (в режиме «одна» — набранную).
- **DoD:**
  - [x] Двойной клик по пустому месту зон «Верх-лево»/«Низ» открывает диалог
    (клик и двойной клик по облачку — прежнее поведение; одиночный клик по
    пустому месту ничего не открывает; двойные клики с модификаторами
    игнорируются; «Верх-право» — никаких действий).
  - [x] Пункт «Добавить мысль» в контекстном меню «Верх-лево»/«Низ» перед
    «Сортировкой»; в «Верх-право» меню без изменений.
  - [x] `Ctrl+Shift+Enter`: single — создать набранную и перевести её в фокус;
    batch — вставить все, в фокус — первую из списка добавляемых (точный
    дубликат связывается, в фокус — он).
  - [x] Спеки 08 (§2.1, §2.7, §4.1, §4.3), 09 (C1, C2) и user-guide дополнены;
    typecheck + клиентские тесты зелёные.
  - [ ] Ручная проверка пользователем.

## L20. Хроника: третий рабочий стол (таблица хронологических комментариев)
- **Статус:** `in_progress` · **Assignee:** zcode · **Зависимости:** L15
- **Источник:** задача «Хроника — новый "рабочий стол"» (мыслесеть «Тест 1»,
  `ETN → План разработки`).
- **Описание:** третий рабочий стол «Хроника» рядом с «Карта мыслей»/
  «Структуры мыслей»: список всех хронологических комментариев с привязками
  к мыслям и связям. Расширение модели: хроно-комментарий получает
  **несколько привязок** (m2m `comment_targets`; первичная остаётся в
  `comments.owner_type/owner_id`). Панель отбора (сохранённые отборы per-view
  на сервере, строка поиска с мини-синтаксисом, поле «мысли» + «+подчинённые»
  до 20 уровней с дедупом, типы мыслей, период, типы связей + способ отбора
  по связям, сортировка, «Применить»/Ctrl+Enter с очисткой истории вида,
  «Очистить отбор», имя + «Сохранить отбор» с перезаписью, «Удалить отбор»).
  Таблица: С|По (серые вне периода)|Заголовок|Мысли/связи|Кратко, сортировка
  valid_from+valid_to+title, пагинация по 50 «Записи X–Y из Z»; навигация
  стрелками по строкам/колонкам/чипам, Tab; контекстные меню строки
  (Добавить — привязки из отбора или HOME / Копировать / Удалить) и чипов
  (Открыть / Отвязать — последняя отвязка → HOME / Связать с… / Добавить к
  выделению / Убрать из выделенных). Нижняя область — как вкладка «Хроника»
  редактора + чипы привязок. Drag-n-drop: на строку — привязка (контроль
  дублей), на шапку/пустое место — новая запись с этой привязкой, на отбор —
  в поле «мысли»; чипы-мысли перетаскиваются в выделение/закрепление/историю.
  Своя история вида (мысли И связи, L4 `chronicle_history`).
- **DoD:**
  - [x] Миграции 019 (`comment_targets` + бэкфилл) и 020 (`saved_filters.view`,
    уникальность имени per (user, view)).
  - [x] Сервер: m2m-операции (создание с 1..N целями, привязка/отвязка,
    перенос primary, последняя отвязка → HOME), обратная совместимость
    старых эндпоинтов, `POST /chronicle/query` (двухфазный отбор: мысли по
    корням+поддереву/типам/ключевым словам в названиях, синонимах и текстах
    комментариев мыслей и их связей с обеих сторон; затем хронозаписи по
    привязкам + способу отбора по связям, пересечение периода, пагинация).
  - [x] Клиент: третий вид (кнопка, хост, L4 `active_view`), панель отбора,
    таблица с навигацией/меню/пагинацией, нижний редактор, drag-n-drop,
    история вида (мысли+связи), realtime-обновление, L4 `chronicle_state`.
  - [x] Тесты: сервер 360 (m2m, отбор, период, scope, миграции) и клиент 159
    (chronicle-state, rest-client) зелёные; REST проверен живьём на временном
    сервере (22/22 сценария, включая m2m и chronicle saved-filters).
  - [x] Спеки 02 (§3.8.1, §3.10.5), 03 (§10.1, §18, §20), 04 (§4.4), 05
    (targets в DTO, MCP без изменений), 08 (§1, §15.1, §17), 09 (блок K),
    11 (L4 `chronicle_state`/`chronicle_history`) и user-guide дополнены.
  - [ ] Ручная проверка пользователем (UI-сценарий §17).

## L21. Иерархические типы

- **Статус:** `done` · **Assignee:** zcode · **Зависимости:** L6, L15, L20
- **Источник:** задача «Иерархические типы» (мыслесеть «Тест 1», `ETN → План разработки`).
- **Описание:** иерархия типов мыслей и связей с наследованием. Корневой тип
  «основной тип» (создаётся автоматически, неудаляем, не назначается
  элементам; его настройки применяются к мыслям/связям без типа). Каждый
  подчинённый тип наследует свойства, иконку, цвета и стили шрифта (для
  связей — цвет/стиль/толщину линии) родителя и может переопределять их;
  вложенность ограничена 4 уровнями, включая корень. В списках — деревья
  (корень всегда развёрнут, остальные свёрнуты). Пока тип используется
  элементами — смена родителя запрещена; очистка родителя = присвоение
  корневого типа; удаление типа с подчинёнными запрещено. Свойства
  наследуются (нельзя менять тип значения/удалять родительские; можно
  переопределять значения по умолчанию, `type_property_overrides`); свойства
  подчинённых типов в редакторе не видны. Отборы по типам (структуры,
  хроника, поиск, `allowed_type_ids` свойства «ссылка на мысль») расширяются
  до поддеревьев (OR).
- **DoD:**
  - [x] Миграция 021: `parent_id`/`is_root` в thought_types/link_types, сид
    корневых типов (фиксированные id) + бэкфилл родителей, `style_set`/
    `width_set` (наследование стиля линии), таблица `type_property_overrides`.
  - [x] Сервер: иерархия типов (лимит 4 уровня, циклы, защита корня,
    запрет репарента при использовании, запрет удаления с детьми), наследуемые
    поля (`NULL` = от родителя, включая font_* в DTO), эффективный список
    свойств по цепочке + override дефолтов, резолв свойств безтиповых
    владельцев по корню, запрет назначения корня мыслям/связям, расширение
    фильтров type_ids до поддеревьев (structure/chronicle/query/search/
    findDuplicates + валидация allowed_type_ids), MCP-каталоги с
    `parent_id`/`is_root`, realtime-события типов.
  - [x] Клиент: деревья в менеджерах типов/выборе типа (combobox)/отборах
    (корень без отметки выбора), выбор родителя в редакторе, секции
    «Свойства типа» и «Унаследованные свойства» (+ override «по умолчанию…»/
    «сбросить»), таблица свойств у типов связей, наследование визуальных
    настроек по цепочке на холсте/в списках, свойства корневого типа у
    безтиповых мыслей, «Сброс» стилей типа = наследовать, realtime-обновление
    каталогов.
  - [x] Тесты: сервер 384 (13 новых: type-hierarchy юнит + REST-сценарий
    иерархии на живом Fastify) и клиент 176 (4 новых type-tree) зелёные.
  - [x] Спеки 02 (§3.3, §3.4.1, §3.7, §3.7.1), 03 (§6.10, §8.1/§8.2, §9,
    §12, §20), 05 (каталоги типов), 08 (§8.1) дополнены.
  - [ ] Ручная проверка пользователем (UI-сценарий §8.1).

## L22. Структуры: команды по отбору

- **Статус:** `done` · **Assignee:** zcode · **Зависимости:** L15
- **Источник:** задача «Выполнение команд с отобранными мыслями» (мыслесеть
  «Тест 1», `ETN → План разработки`,
  `087d0ad6-c562-4434-8cde-4c1d96c09a20`).
- **Описание:** кнопка «Команды ▾» рядом с «Применить»/«Очистить» панели
  отбора «Структур» открывает меню массовых команд над **всеми** мыслями
  последнего применённого отбора — без учёта пагинации результатов (id
  собираются запросом `ids_only`, страницы по 2000). Команды: добавить к
  выделенным / удалить из выделенных (общая панель выделения); сделать
  актуальными / неактуальными; изменить тип… (диалог с «Без типа»);
  добавить родительские мысли… / добавить подчинённые мысли… (диалог выбора
  мыслей; новые связи **без типа**, уже связанная пара не меняется);
  сделать единственными родителями… (удалить все входящие, кроме выбранных;
  подтверждение); разорвать связи с родителями… / с подчинёнными… (диалог
  выбора + подтверждение; удаляются связи только с выбранными); удалить
  (подтверждение; защищённая HOME — в списке неудач). Пустой отбор — тоже
  отбор (HOME + сироты). Уведомление с числом применённых/неудач, дерево
  перезапрашивается.
- **DoD:**
  - [x] Сервер: 5 новых op `thoughts/batch` (`link_parents`, `link_children`,
    `set_only_parents`, `unlink_parents`, `unlink_children`, §6.6) —
    идемпотентны по парам, self-loop пропускается; `incomingLinksOf` в
    link-service.
  - [x] Сервер: `ids_only: true` у `POST /thoughts/query` (§6.10) — тот же
    набор кандидатов/порядок, только id, потолок limit 2000; общий
    построитель WHERE (`buildFilterQuerySql`) для ref- и id-выборки.
  - [x] Клиент: REST `queryStructureThoughtIds` + IPC `structures.queryIds`;
    `Client-Request-Id` на каждый вызов `thoughts.batch`; кнопка
    «Команды ▾» (переносится на узкой панели) + меню и исполнение
    (`screens/structures/commands.ts`); команды действуют на применённый
    отбор (`appliedQuery`); `removeFromSelection` + чанковый resolve
    (по 100) длинного списка выделения.
  - [x] Тесты: сервер 394 (новые: REST bulk-link сценарий, ids-only
    эквивалентность ref-выборке), клиент 185 — зелёные; typecheck чистый.
  - [x] Спеки 03 (§6.6, §6.10) и 08 (§15.3, §15.3.1) дополнены.
  - [ ] Ручная проверка пользователем.

## L23. Структуры: «Найти на карте мыслей» в контекстном меню мысли

- **Статус:** `done` · **Assignee:** zcode · **Зависимости:** L15
- **Источник:** задача «в контекстное меню мысли на экране "Структуры
  мыслей" добавить команду "Найти на карте мыслей"» (мыслесеть «Тест 1»,
  `ETN → План разработки`,
  `4470c50c-175b-4997-8980-30dd562791be`).
- **Описание:** в контекстном меню мысли в виде «Структуры мыслей» новый
  пункт «Найти на карте мыслей» (сразу после «Открыть редактор»): по
  нажатии открывается вид «Карта мыслей» с фокусом на этой мысли —
  переключение вида (`setActiveView('map')`) + обычная смена фокуса
  (`setFocus`: FLIP-анимация, история фокуса, sticky-сброс редактора).
  В остальных контекстных меню мысли (холст, панель выделения,
  закреплённые) пункта нет — он опционален в общем построителе меню
  (колбэк `findOnMapHandler`, как `openHandler`).
- **DoD:**
  - [x] Клиент: пункт меню включается колбэком `findOnMapHandler` в
    `showThoughtContextMenu`; передаётся только из дерева «Структур».
  - [x] Тесты: новый `client/tests/context-menu.test.ts` — зелёные;
    typecheck чистый.
...
## L26. Копирование и вставка мыслей (между мыслесетями)

- **Статус:** `done` · **Assignee:** zcode · **Зависимости:** —
- **Источник:** задача «Копирование и вставка мыслей» (мыслесеть «Тест 1»,
  `ETN → План разработки`, `bb8277f6-ba25-427d-a32f-9e9884e0af2d`).
- **Описание:** сейчас в контекстном меню мысли есть команды «Копировать» /
  «Вырезать» / «Вставить», но они делают совсем не то, что нужно: «Вставить»
  лишь создаёт одну связь от скопированной мысли к целевой. Полная
  реализация: копировать можно одну мысль (`Ctrl+C`, контекстное меню) или
  выделенные мысли (новая команда «Скопировать» в меню «Действия» панели
  выделенных); при копировании нескольких — копируются и связи между
  выделенными; вставка работает в облачко мысли (на карте или в структурах)
  и в любой комментарий.
- **Что копируется:** title, синонимы, актуальность, настройки стиля
  (цвета/font_*), иконка (emoji или URL/attachment-id), тип мысли (id+имя
  для резолва), постоянный комментарий, значения свойств (для
  `thought_ref` — только id+имя для резолва), вложения (только URL/file_path,
  без файлов), связи между выделенными мыслями (цвета/стили, постоянный
  комментарий, вложения, тип связи — id+имена). Не копируются: id, файлы
  иконок/вложений, настройки типов мыслей/связей, хронологические
  комментарии.
- **Вставка:**
  - *В облачко мысли* — все скопированные мысли становятся дочерними мысли,
    в контекстном меню которой вызвана вставка; связи между ними
    пересоздаются с новыми id; типы и `thought_ref`-значения ищутся сначала
    по id, затем по имени (если ничего не найдено — поле очищается);
    `[[#<id>]]`-ссылки в комментариях остаются как есть в той же сети и
    переделываются на `[[n:<net>#<id>]]` при кросс-сети.
  - *Произвольный текст в облачко* — если строка целиком UUID, ищется мысль
    по этому id и создаётся связь от облачка к найденной мысли; иначе
    создаётся новая мысль (до `|` — имя, после — синонимы через запятую);
    многострочно — каждая строка отдельно.
  - *В комментарий* — для каждой мысли вставляется `[[#<id>]]` (или
    `[[n:<net>#<id>]]` при кросс-сети) через запятую.
- **DoD:**
  - [x] Сервер: новый `POST /networks/:nid/thoughts/copy-batch` в одной
    транзакции (atomic): резолв типов/thought_ref (по id → по имени),
    создание мыслей, связей, копирование вложений (L25), постоянных
    комментариев, перезапись wiki-ссылок в комментариях при кросс-сети.
    Возвращает id-маппинг (старый → новый) + новые сущности для realtime.
  - [x] Shared: `ThoughtCopyInput`/`ThoughtCopyItem`/`ThoughtCopyResult`
    (множество мыслей, parent_id, тип-резолв-режим).
  - [x] IPC: `etn.thoughts.copyBatch` (contract, handlers, rest-client,
    preload). Идемпотентность через `Client-Request-Id`.
  - [x] Клиент: новый модуль `clipboard.ts` (multi-thought clipboard с
    `sourceNetworkId`, `thoughts: Map<id, snapshot>`, `links: link[]`,
    подписки для UI-обновлений).
  - [x] Контекстное меню мысли: убрать «Вырезать»; «Копировать»
    копирует одну мысль; «Вставить» вставляет буфер обмена под выбранную
    мысль (или парсит произвольный текст, если буфер пуст и есть
    text-содержимое).
  - [x] Панель выделенных: команда «Скопировать мысли» в меню «Действия»,
    копирующая весь текущий список выделенных (с их связями между собой).
  - [x] `Ctrl+C`/`Ctrl+V` на холсте/структурах (не в редакторе комментария):
    `Ctrl+C` копирует фокус или выделение, `Ctrl+V` вставляет под фокус.
  - [x] Вставка в комментарий (CM6) — как plain-text «ссылка на id»
    (`[[#<id>]]`) в позицию курсора.
  - [x] Спеки `03 §6.13`, `08 §4.5`, `11` дополнены отдельным
    `docs:`-коммитом (`9ef81e5`).
  - [x] `npm run typecheck` + серверные тесты зелёные; ручная проверка
    single/multi copy, кросс-сети, paste в облачко/комментарий.

## L25. Использование одного вложения в нескольких мыслях

- **Статус:** `todo` · **Assignee:** zcode · **Зависимости:** —
- **Источник:** задача «Использование одного вложения в нескольких мыслях»
  (мыслесеть «Тест 1», `ETN → План разработки`,
  `1764f9bf-b0cd-404c-b97e-720ffb75b2af`).
- **Описание:** сейчас, чтобы использовать один файл/ссылку в нескольких мыслях,
  пользователь повторно вставляет его через диалог добавления — для
  серверного хранилища это плодит копии в `networks/<nid>/attachments/`,
  перерасходуя диск (для клиентских путей — менее критично, но визуально
  несколько строк с одним файлом сбивают с толку). Решение: два новых пути
  переиспользования вложения без копирования файла.
  - **Контекстное меню вложения → «Скопировать в мысли…»** — мульти-выбор
    мыслей (`pickThoughtsDialog` без создания); серверный эндпоинт
    `POST /attachments/{id}/copy` создаёт по новой строке `attachments` на
    каждую выбранную мысль с теми же видимыми полями
    (kind/url/file_path/mime_type/title/description/icon/file_size). Файл не
    дублируется, общее правило §11 «файл удаляется, только когда никем не
    используется» работает as-is.
  - **Диалог добавления вложения → вкладка «Найти существующее»** — поиск
    по вложениям сети (`GET /attachments?q=…`, исключая вложения текущего
    владельца); клик по найденной строке создаёт у текущего владельца новую
    строку с теми же полями, что и источник.
  - В обоих случаях: дубликат в целевой мысли (тот же kind + тот же
    url/file_path) пропускается тихо; по окончании — отчёт «Скопировано /
    использовано N, пропущено M».
  - Дополнительно: MCP-инструменты `etn.attachments.copy` и
    `etn.attachments.search` для консистентности с существующим
    `etn.attachments.add` (05-mcp-server.md §4.2).
- **DoD:**
  - [ ] Сервер: `copyAttachment` и `searchAttachments` в
    `attachment-service.ts`; маршруты `POST /attachments/{id}/copy` и
    `GET /attachments` (с `q`/`exclude_owner`/`kind`/`limit`/`offset`);
    эмиссия `attachment.created` на каждую созданную строку.
  - [ ] Shared: `AttachmentCopyInput`, `AttachmentCopyResult`,
    `AttachmentSearchQuery`.
  - [ ] Клиент: `RestClient.copyAttachment`/`searchAttachments`,
    `EtnApi.attachments.copy`/`search`, pass-through handlers.
  - [ ] UI: пункт «Скопировать в мысли…» в контекстном меню вложения;
    вкладка «Найти существующее» в диалоге добавления (дебаунс 250 мс,
    исключение владельца); `notice` с отчётом «Скопировано N, уже было M».
  - [ ] MCP: `etn.attachments.copy`, `etn.attachments.search` (параметры,
    схемы, запись в §4.2).
  - [ ] Спеки: 03 §11, 08 §6.5/§6.5.1, 05 §4.2 (отдельным `docs:`-коммитом).
  - [ ] Тесты сервера: `copyAttachment` (создание N строк, пропуск дубликатов,
    422 на несуществующий target, атомарность); `searchAttachments` (фильтр по
    `q`/`kind`/`exclude_owner`, ключевые слова с `-исключением`); интеграционные
    через `app.inject` для `POST /attachments/:id/copy` и `GET /attachments`.
  - [ ] `npm run typecheck` + серверные тесты зелёные.
  - [ ] Ручная проверка пользователем.
