...
- **Источник:** задача «Упростить добавление мыслей» (мыслесеть «Тест 1»,
  `ETN → План разработки`).
- **Описание:** три новых пути добавления мыслей на холсте: (1) клик по пустому
  месту зоны «Верх-лево» открывает диалог добавления родительской мысли,
  зоны «Низ» — дочерней (владелец вызова — фокус); в «Верх-право» добавления
  нет — у фокуса может быть несколько родителей; (2) в контекстном меню зон
  «Верх-лево»/«Низ» появился пункт «Добавить мысль» (в меню «Верх-право»
  пункта нет, сортировка — как раньше); (3) `Ctrl+Shift+Enter` в диалоге
  добавления вставляет всё и сразу переводит в фокус первую из добавляемых
  мыслей (в режиме «одна» — набранную).
- **DoD:**
  - [x] Клик по пустому месту зон «Верх-лево»/«Низ» открывает диалог (клик по
    облачку — прежнее поведение; клик после drag-жеста и клики с модификаторами
