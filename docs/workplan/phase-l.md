...
- **DoD:**
  - [ ] Сервер: новый `POST /networks/:nid/thoughts/copy-batch` в одной
  ...
  - [ ] Shared: `ThoughtCopyInput`/`ThoughtCopyItem`/`ThoughtCopyResult`
    (множество мыслей, parent_id, тип-резолв-режим).
  - [ ] IPC: `etn.thoughts.copyBatch` (contract, handlers, rest-client,
    preload). Идемпотентность через `Client-Request-Id`.
  - [ ] Клиент: новый модуль `clipboard.ts` (multi-thought clipboard с
    `sourceNetworkId`, `thoughts: Map<id, snapshot>`, `links: link[]`,
    подписки для UI-обновлений).
  - [ ] Контекстное меню мысли: убрать «Вырезать»; «Копировать»
    ...
    text-содержимое).
  - [ ] Панель выделенных: команда «Скопировать мысли» в меню «Действия»,
    копирующая весь текущий список выделенных (с их связями между собой).
  - [ ] `Ctrl+C`/`Ctrl+V` на холсте/структурах (не в редакторе комментария):
    `Ctrl+C` копирует фокус или выделение, `Ctrl+V` вставляет под фокус.
  - [ ] Вставка в комментарий (CM6) — как plain-text «ссылка на id»
    (`[[#<id>]]`) в позицию курсора.
  - [ ] Спеки `03 §6.13`, `08 §4.5`, `11` дополнены отдельным
    `docs:`-коммитом.
  - [ ] `npm run typecheck` + серверные тесты зелёные; ручная проверка
