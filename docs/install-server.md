...
Должно вернуть `{"status":"ok","version":"0.7.1","uptime":...}`.
...
> ⚠️ **Перед обновлением на 0.6.5:** сделайте резервную копию `data.db`
> каждой сети. Миграция 032 переводит свойства из определений внутри
> типа в самостоятельные сущности справочника сети — операция
> необратима (предпросмотр на сети ETN: 18 определений → 15 свойств,
> переименований нет). Перед перезапуском сервера на новой версии
> убедитесь, что бэкап свежий и проверен.
